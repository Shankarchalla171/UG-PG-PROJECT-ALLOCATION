...
const Navbar = () => {
  const {authDispatch, role}=useContext(AuthContext);
    ...
    <header className="sticky top-0 z-50 flex w-full justify-between items-center px-6 py-3 bg-gradient-to-r from-amber-50 via-orange-50 to-amber-50 text-amber-900 border-b border-orange-200/60 shadow-sm hover:cursor-pointer">
      ...
      <nav className="flex items-center gap-1">
        ...
        <button
          ...
          className="px-4 py-2 text-sm font-medium text-white bg-red-500/20 backdrop-blur-sm border border-red-500/30 rounded-lg hover:bg-amber-500/30 hover:border-amber-500/50 focus:outline-none focus:ring-2 focus:ring-amber-500/50 transition-all duration-200"
        >
          Logout
        </button>
      </nav>
